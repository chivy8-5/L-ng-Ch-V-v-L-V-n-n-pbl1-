...
private:
    ...
    string masinhvien;
    string email;
    string khoahoc; // Vẫn giữ là protected
    string ngaydangky;
    string trangthai;
...
public:
    ...
    string getKhoahoc() const { return khoahoc; } // Phương thức công cộng để truy cập khoahoc
    // Các phương thức khác ở đây...
void setKhoahoc(const string& khoahoc) {
...
void nhapthongtinsinhvien(  vector<Sinhvien> &sinhviens, const vector<Khoahoc> &khoahocs)
{
    Sinhvien sv;
    sv.nhapthongtin();

    // Hiển thị danh sách các khoá học để cho sinh viên chọn
    cout << "Danh sach cac khoa hoc:" << endl;
    for (int i = 0; i < khoahocs.size(); i++)
    {
        cout << i + 1 << ". " << khoahocs[i].gettenkhoahoc() << endl;
    }

    // Nhập số thứ tự của khoá học mà sinh viên muốn đăng ký
    int choice;
    cout << "Nhap so thu tu cua khoa hoc muon dang ky: ";
    cin >> choice;

    // Kiểm tra nếu lựa chọn hợp lệ
    if (choice >= 1 && choice <= khoahocs.size())
    {
        sv.setKhoahoc(khoahocs[choice - 1].gettenkhoahoc()); // Sử dụng phương thức setKhoahoc để thiết lập giá trị cho khoahoc
        sinhviens.push_back(sv);
        cout << "Dang ky khoa hoc thanh cong!" << endl;
    }
    else
    {
...
// Menu LỚP SINH VIÊN
// Menu LỚP SINH VIÊN
void Menusinhvien(vector<Sinhvien> &sinhviens, const vector<Khoahoc> &khoahocs)
{
    int choice;
    while (true)
    {
        ...
        if (choice == 1)
        {
            nhapthongtinsinhvien(sinhviens, khoahocs);
        }
        else if (choice == 2)
        {
...
int main()
{
    ...
    if (tenTaiKhoan.length() > 0)
    {
        ...
        while (true)
        {
            ...
            {
                Menusinhvien(sinhviens, khoahocs);
                file = "sinhviens.txt";
                writeToFile(file, sinhviens);
            }
            else if (choice == 3)
            {
                cout << "cac ban co the tim hieu file gihub o day: https://github.com/chivy8-5/L-ng-Ch-V-v-L-V-n-n-pbl1-";
                system("pause");
                return 0;
            }
        }
